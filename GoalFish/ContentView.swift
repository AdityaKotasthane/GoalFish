...
struct ContentView: View {
    ...
    @State private var isShowingBackgroundSelection = false // Track showing background selection view
    @State private var selectedBackground: String? = nil // Track selected background
    
    var body: some View {
        NavigationView {
            ZStack {
                // Background image
                Image(selectedBackground ?? "background") // Use selected background if available, otherwise use default background
                    .resizable()
                ...
                VStack {
                    AnimatedView(fileName: "splashscreen.json")
                        .frame(
                            ...
                            height: 100 // Keep the height at 100
                        )
                        .padding(.bottom, 10) // Add padding to create space between logo and animation
                        .offset(y: UIScreen.main.bounds.height * 0.4) // Offset to move the animation to the bottom of the screen
                ...
                if isShowingBackgroundSelection {
                    VStack {
                        ScrollView(.horizontal) {
                            LazyHStack(spacing: 20) {
                                ForEach(0..<5) { index in
                                    // Display background tiles
                                    Image("background\(index + 1)") // Assuming your backgrounds are named background1, background2, etc.
                                        .resizable()
                                        .aspectRatio(contentMode: .fit)
                                        .frame(width: 100, height: 100)
                                        .cornerRadius(10)
                                        .onTapGesture {
                                            selectedBackground = "background\(index + 1)"
                                            isShowingBackgroundSelection.toggle()
                                        }
                                }
                            }
                            .padding()
